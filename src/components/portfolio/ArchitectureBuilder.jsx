...
export function ArchitectureBuilder() {
  ...
  const validate = () => {
    ...
    if (missing.length === 0) {
      ...
      setResult("bad");
      setReason(
        `Missing critical component${missing.length > 1 ? "s" : ""}: ${missing.join(", ")}. Traffic can't reach a resilient data tier.`
      );
    }
  };
  ...
  return (
    <section id="builder" className="py-16">
      <div className="mx-auto w-[min(1200px,94vw)]">
        <SectionHeader
          eyebrow="/challenge"
          title="AWS Architecture Builder"
          sub="Drag services onto the canvas (or tap to add). Design a resilient stack, then validate."
        />
        ...
        <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
              ...
              {services.map((s) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => toggle(s.id)}
                    className={`cursor-grab rounded-xl border p-3 transition hover:-translate-y-0.5 active:cursor-grabbing ${
          ...
          >
            ...
            <div className="mt-6 flex items-center gap-2">
              ...
              <button
                onClick={reset}
                className="flex items-center gap-1.5 rounded-xl border border-border bg-secondary/60 px-3 py-2 text-sm"
              >
            ...
            {result && (
              <motion.div
                ...
                className={`mt-4 flex items-start gap-2 rounded-xl border p-3 text-sm ${
                  result === "ok"
                    ? "border-[var(--cloud-green)]/40 bg-[var(--cloud-green)]/10 text-[var(--cloud-green)]"
              ...
              >
                ...
                <div>
                  <div className="font-semibold">
                    {result === "ok" ? "Architecture Valid" : "Needs work"}
                  </div>
