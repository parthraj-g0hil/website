      ...
      <div className="mx-auto w-[min(1200px,94vw)]">
        ...
        <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
          <div className="flex flex-col gap-2">
            {projects.map((x) => <button
    key={x.id}
    onClick={() => setActive(x.id)}
    className={`glass flex items-start justify-between gap-2 rounded-2xl p-4 text-left transition hover:-translate-y-0.5 ${active === x.id ? "ring-1 ring-[var(--cloud-blue)]/50" : ""}`}
  >
                <div>
                  <div className="font-mono text-[10px] text-muted-foreground">{x.timeline}</div>
                  <div className="mt-0.5 font-semibold">{x.name}</div>
                </div>
                <ChevronRight className={`h-4 w-4 shrink-0 transition ${active === x.id ? "text-[var(--cloud-cyan)]" : "text-muted-foreground"}`} />
              </button>)}
          </div>
  ...
  >
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-white" style={{ background: "var(--gradient-primary)" }}>
              ...
              <div className="mt-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {p.stack.map((s) => <span key={s} className="rounded-md border border-border bg-secondary/60 px-2 py-0.5 font-mono text-[11px]">
                      {s}
                    </span>)}
                </div>
              </div>
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                {p.metrics.map((m) => <div key={m.label} className="rounded-xl border border-border bg-secondary/40 p-3 text-center">
