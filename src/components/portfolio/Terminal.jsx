...
export function InteractiveTerminal() {
  const [lines, setLines] = useState(banner.map((t) => ({ text: t, color: "cyan" })));
  ...
  const run = (raw) => {
    ...
    const add = (...ls) =>
      ls.forEach((l) => out.push(typeof l === "string" ? { text: l } : l));
    ...
    switch (cmd) {
      ...
      case "help":
        ...
        break;
      case "about":
        add(`> ${profile.name}`, `> ${profile.role} · ${profile.location}`, "", profile.summary);
        break;
      case "skills":
        add("→ Navigating to skills...");
        skills.slice(0, 6).forEach((s) =>
          add({ text: `  • ${s.name.padEnd(14)} ${s.category} (${s.years}y)`, color: "green" })
        );
      ...
      case "aws":
        add(
          ...
          "Services:   EC2 · VPC · RDS · S3 · Lambda · ECS · CloudFront · WAF · Route53 · IAM Identity Center"
        );
        break;
      case "docker":
        add(
      ...
      <div className="mx-auto w-[min(1000px,94vw)]">
        ...
        >
          ...
          <div ref={bodyRef} className="scrollbar-thin h-[380px] overflow-auto p-4 font-mono text-[13px] leading-6">
            {lines.map((l, i) => (
              <div key={i} className={colorClass(l.color)}>
