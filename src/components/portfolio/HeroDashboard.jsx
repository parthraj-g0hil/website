...
import {
  Server, Rocket, Box, Boxes, Activity, Cpu, MemoryStick,
  Users, CheckCircle2, RefreshCw, TrendingUp,
} from "lucide-react";
import { profile } from "@/lib/portfolio-data";

const initial = [
  { icon: Server, label: "Running Servers", value: "24", sub: "us-east-1 · ap-south-1", color: "cloud-blue" },
  ...
  { icon: Users, label: "Visitors Online", value: "1", sub: "you 👋", color: "cloud-amber" },
...
function Sparkline({ color }) {
  const [pts] = useState(() => Array.from({ length: 20 }, () => 20 + Math.random() * 40));
...
export function HeroDashboard() {
  ...
  const runAction = (label) => {
    ...
    const steps =
      label === "Deploy App"
        ? deploySteps
        : label === "Scale Cluster"
        ? ["Fetching HPA metrics...", "Adding 2 nodes...", "Warming pods...", "Cluster scaled ✓"]
        : ["Draining connections...", "Stopping service...", "Starting service...", "Health check OK ✓"];
  ...
  return (
    <section id="dashboard" className="relative pt-32 pb-16 grid-bg">
      <div className="mx-auto w-[min(1200px,94vw)]">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {["Deploy App", "Scale Cluster", "Restart Service"].map((a, i) => (
              <button
                key={a}
                onClick={() => runAction(a)}
                disabled={running}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <Icon className="h-4 w-4" style={{ color: `var(--${s.color})` }} />
                  <span className="h-1.5 w-1.5 rounded-full bg-[var(--cloud-green)] pulse-ring" />
                </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-3">
          ...
          >
            ...
            <div className="scrollbar-thin h-[112px] space-y-1 overflow-auto font-mono text-xs">
              {log.length === 0 && (
                <div className="text-muted-foreground">▸ Click an action button to trigger a workflow.</div>
              )}
              <AnimatePresence>
                {log.map((s, i) => (
                  <motion.div
                  ...
                  >
                    {s.done ? (
                      <CheckCircle2 className="h-3 w-3 text-[var(--cloud-green)]" />
                    ) : s.active ? (
                      <RefreshCw className="h-3 w-3 animate-spin text-[var(--cloud-cyan)]" />
                    ) : (
