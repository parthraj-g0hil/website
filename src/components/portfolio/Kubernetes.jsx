...
function KubernetesViz() {
  ...
  return <section id="k8s" className="py-16">
    ...
    return <div key={n} className="rounded-xl border border-border bg-secondary/30 p-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <AnimatePresence>
                        {nodePods.map((p) => <motion.div
      key={p.id}
      initial={{ scale: 0, opacity: 0 }}
      animate={{ scale: 1, opacity: 1 }}
      exit={{ scale: 0, opacity: 0 }}
      transition={{ type: "spring", stiffness: 260, damping: 22 }}
      className={`aspect-square rounded-lg border ${p.status === "Running" ? "border-[var(--cloud-blue)]/40 bg-[var(--cloud-blue)]/15" : "border-[var(--cloud-amber)]/40 bg-[var(--cloud-amber)]/15"} grid place-items-center`}
      title={p.id}
    >
                    ...
                    {nodePods.length === 0 && <div className="py-3 text-center font-mono text-[10px] text-muted-foreground">idle</div>}
                  </div>;
  })}
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-5 font-mono text-xs">
            ...
            <div className="space-y-1 text-muted-foreground">
              ...
              <div className="text-foreground">deploy/web       {pods.filter((p) => p.status === "Running").length}/{replicas}     <span className="text-[var(--cloud-green)]">Healthy</span></div>
              <div className="text-foreground">svc/web-lb       LoadBalancer   <span className="text-[var(--cloud-cyan)]">EXTERNAL</span></div>
