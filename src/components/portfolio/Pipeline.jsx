...
function Pipeline() {
  ...
  return <section id="pipeline" className="py-16">
  ...
  >
    ...
    return <div key={s.label} className="relative">
                  <motion.div
      animate={{
        scale: isActive ? 1.04 : 1,
        boxShadow: isActive ? "0 0 0 2px oklch(0.72 0.18 240 / 0.6)" : "0 0 0 1px transparent"
      }}
      className={`rounded-xl border p-3 transition ${done ? "border-[var(--cloud-green)]/40 bg-[var(--cloud-green)]/10" : isActive ? "border-[var(--cloud-blue)]/50 bg-[var(--cloud-blue)]/10" : "border-border bg-secondary/40"}`}
    >
                    <div className="flex items-center gap-2">
                      {done ? <CheckCircle2 className="h-4 w-4 text-[var(--cloud-green)]" /> : isActive ? <RefreshCw className="h-4 w-4 animate-spin text-[var(--cloud-blue)]" /> : <s.icon className="h-4 w-4 text-muted-foreground" />}
      ...
      className="h-full"
      style={{
        background: done ? "var(--cloud-green)" : "var(--gradient-primary)"
      }}
    />
                    </div>
                  </motion.div>
                  {i < stages.length - 1 && <div className="pointer-events-none absolute top-1/2 -right-2 hidden h-px w-4 bg-border lg:block" />}
                </div>;
  })}
          </div>
          {active >= stages.length && <motion.div
